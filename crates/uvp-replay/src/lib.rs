...
pub fn replay_chain_events(mut events: Vec<Value>, options: &ReplayOptions) -> Result<Value> {
    ...
    for event in &events {
        ...
        match event_name {
            ...
            // 的出生通道判别）。
            "OrderTriggered" => {
                let order_key = order_key(value_str(event, "planId")?, value_str(event, "orderId")?);
                state.order_trigger_tx.insert(
                    order_key,
                    value_str(event, "transactionHash")?.to_string(),
                );
            }
...
struct OracleState {
    ...
    order_trigger_tx: BTreeMap<String, String>,
}
...
fn hook_observation_key(observation: &Value) -> String {
    format!(
        "{}::{}::{}",
        observation
            .get("planId")
...
fn same_due_at(left: Option<&str>, right: Option<&str>) -> bool {
    match (left, right) {
        (None, None) => true,
